...
// content fields are materialized into it. Generated by codegen directly from
// resource.revisioned_in, resolved and validated against the actual registered
// wrapper CRD at codegen time (same pass that produces the sidecar DDL and
// GetContentIndexedKeyValuePairs). A storage backend never re-derives this by
// reflecting into proto options at runtime — see ContentIndexDescribable.
...
// ContentIndexable. It is generated straight from the same parsed
// revisioned_in annotation that produces the sidecar DDL and
// GetContentIndexedKeyValuePairs, with the wrapper kind already resolved (and
// validated against the real wrapper CRD) at codegen time. Storage backends
// call this instead of reflecting into MessageOptions at startup, so the DDL,
// the extractor, and the runtime read/write routing can never drift apart.
