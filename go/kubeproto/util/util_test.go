...
func TestCollectInt64FieldsWithMap(t *testing.T) {
	type Inner struct {
	...
	paths := util.CollectInt64Fields(reflect.TypeOf(WithMap{}))
	assert.Equal(t, []string{"items.*.count"}, paths)
}

func TestCollectInt64Fields(t *testing.T) {
	type Nested struct {
		Count   int64  `json:"count"`
	...
	type Root struct {
		...
		MapVals  map[string]int64  `json:"mapVals"`
		MapObjs  map[string]Nested `json:"mapObjs"`
		Slice    []Nested          `json:"slice"`
	}

	paths := util.CollectInt64Fields(reflect.TypeOf(Root{}))
...
func TestUnquoteInt64Fields(t *testing.T) {
	tests := []struct {
		name  string
		input string
		paths []string
		...
		{
			name:  "uint64 quoted",
			input: `{"count":"18446744073709551615"}`,
			paths: []string{"count"},
			want:  `{"count":18446744073709551615}`,
		},
		{
			name:  "max int64 boundary",
			input: `{"count":"9223372036854775807"}`,
			paths: []string{"count"},
			want:  `{"count":9223372036854775807}`,
		},
		{
			name:  "max int64 + 1 (valid uint64)",
			input: `{"count":"9223372036854775808"}`,
			paths: []string{"count"},
			want:  `{"count":9223372036854775808}`,
		},
		{
		...
		{
			name:  "non-integer string left unchanged",
			input: `{"count":"not-a-number"}`,
			paths: []string{"count"},
			want:  `{"count":"not-a-number"}`,
		},
		{
			name:  "map values",
			input: `{"m":{"a":"7","b":"8"}}`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := util.UnquoteInt64Fields([]byte(tt.input), tt.paths)
...
func TestUnquoteInt64FieldsWithMap(t *testing.T) {
	type Inner struct {
		Count int64  `json:"count"`
		Label string `json:"label"`
	}
	...
	input := []byte(`{"items":{"a":{"count":"7","label":"x"},"b":{"count":"8","label":"y"}}}`)
	paths := util.CollectInt64Fields(reflect.TypeOf(WithMap{}))
	got, err := util.UnquoteInt64Fields(input, paths)
	assert.NoError(t, err)

	var result WithMap
	err = json.Unmarshal(got, &result)
	assert.NoError(t, err)
	assert.EqualValues(t, 7, result.Items["a"].Count)
	assert.EqualValues(t, 8, result.Items["b"].Count)
}
