...
Module loading/forward-signature helpers, and the ONNX export machinery
(MHA fused-fastpath disable, dynamo-with-legacy-fallback, IO shape
normalization from the model schema, and batch-size expansion) that back the
public fusing API. Nothing here is part of the public interface — import from
``fuse`` instead.
"""
